...
mod tests {
    ...
    fn v16_isa_and_disasm_annotation() {
        // The assembler accepts the v1.6/1.6.2 ISA (same level the disassembler
        // decodes); gas's default v1.2 silently rejected these.
        let nx = NyxstoneTricoreGCC::new().unwrap();
        ...
        // Disassembly carries no objdump-style " <0x..>" address annotation.
    ...
    fn reloc_offset_is_absolute_and_address_hint_resolves() {
        // Regression: reloc offset must be the absolute site address
        // (base + section offset), and symbol.address must carry the matching
        // LabelDefinition's address through the binding (a leading-dot label
        // name like ".lbl" must match too).
        let nx = NyxstoneTricoreGCC::new().unwrap();
        let (_, relocs) = nx.assemble_with_relocs(
